...
class Wchat extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $this->instance_id = 0;
        $data=Db::name('wxconfig')->where(['id'=>1])->find();
    ...
    public function getWeixinMediaDetailByMediaId($media_id){
        ...
        $item_list=Db::name('wx_media_item')->where(['id' => $media_id])->find();
        if (!empty($item_list)) {
            // 主表
            // $weixin_media = db('wx_media');
            // $weixin_media_info["media_parent"] = $weixin_media->where(["media_id" => $item_list["media_id"] ])->find();
            $weixin_media_info["media_parent"] =Db::name('wx_media')->where(["media_id" => $item_list["media_id"] ])->find();
            // 微信配置
            ...
            $weixin_media_info["weixin_auth"]=Db::name('wx_media')->where(["instance_id" => $weixin_media_info["media_parent"]["instance_id"]])->find();
            $weixin_media_info["media_item"] = $item_list;
            // 更新阅读次数
            $res = Db::name('wx_media_item')->where(["id" => $media_id])->update(["hits" => ($item_list["hits"] + 1)]);
